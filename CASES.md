...
Real cases where drift was detected and corrected in actual work. Trigger
attribution is evidence-specific: some were caught by ACGM, some by a user or
external audit, and one involved a third-party gate. Hook activity is not silently
rewritten as an ACGM win. All project names, code symbols, infrastructure names,
versions, schema hints, and paths are **removed** — only the drift mechanism
remains. Generic, well-known technical terms may stay.
...
| # | Title | Drift | Trigger |
|---|---|---|---|
| 1 | Migration script silently drops data | ② → ① | third-party pre-Bash gate + grounding |
...
**Correction.** A third-party pre-execution gate held the first command — it did
not run. That mechanical hold was **not an ACGM hook**, and this case must not claim
otherwise. The subsequent read-only grounding was consistent with ACGM's
truth-first discipline: instead of trusting the stale handoff, it checked both
real stores and **read the script's source** (not guessed). The copy loop and the
...
**Correction.** External human audit caught the chain. The later PreToolUse
mechanism was a **post-incident response**, not what caught this case; its presence
cannot be counted retroactively as a win. The durable change is the three-layer
model plus an evidence-maturity system that separates an observed incident from a
corroborated general rule.

**Fix.** The operational-truth and pre-destructive-grounding ideas are consolidated
into one high-risk evidence gate (target / current state / authority and rollback /
postcondition). "Async self-monitoring" becomes a post-action verification
obligation with an explicit `pending → verified` boundary. Compaction
re-verification and the ritual-compliance criterion remain; mechanisms cover only
the mechanically-checkable subset and the audit layer closes the rest.
...
|---|---|---|
| 1 | third-party pre-Bash gate + grounding | hold the command → read real stores → read source → derive consequence |
...
| 9 | external human audit | assisted correction; evidence maturity + high-risk gate + post-action verification |
...
## How an Event Ledger counts a governance outcome

A case is not evidence of an ACGM win until attribution is established. Record at
least:

- **initiator:** hook / agent / user / external audit;
- **phase:** before the risky action, after it, or during later audit;
- **outcome:** blocked, corrected, allowed, unresolved, or false positive;
- **verification:** the postcondition evidence that closed the event.

Hook activation, skill invocation, and principle citation are activity only.
`Prevented` requires a pre-action intervention plus verification that the risk did
not land. If the user or an external auditor found it first, it is an assisted
correction, not an autonomous catch. A repair is not complete until its
postcondition passes, and false positives remain visible.
...
实际工作中漂移被发现并纠正的真实案例。触发归因逐案记录:有些由 ACGM 抓到,有些由
用户或外部审计发现,还有一个由第三方 gate 拦下。不能把 hook 活动静默改写成 ACGM
战绩。所有项目名、代码符号、基础设施名、版本、schema 提示、路径均已**移除**——
只留漂移机制。通用、公开熟知的技术名词可保留。
...
| # | 标题 | 漂移 | 触发 |
|---|---|---|---|
| 1 | 迁移脚本静默丢数据 | ② → ① | 第三方执行前 gate + grounding |
| 2 | 凭名字假设组件行为 | ② | 真值核查 |
...
**纠错。** 第三方执行前 gate 挡住首条命令——没让它跑。这个机械拦截**不是 ACGM
hook**,本案例不得把功劳归给 ACGM。随后只读 grounding 的做法符合 ACGM 的真值优先:
没有信过期交接文档,而是核对两端真实存储,并**读了脚本源码**(没猜)。拷贝循环和校验
循环用的是同一份坏列表,不在列表里的一律跳过。后果链:错名被跳过;真表不在列表里,
**根本不会被遍历** → 它的数据行会被静默丢弃。更糟:脚本自带的双向计数校验用的是
*同一份*坏列表,查不到这次丢失,会照常打印成功、全绿。
...
**纠错。** 外部人审抓到这条链。后来新增的 PreToolUse 机制是**事后响应**,不是本案例
当时的触发点,不能倒过来算成一次战绩。真正可保留的变化是三层结构,以及把一次已观察
事故和一条已佐证通用规则分开的证据成熟度体系。

**修复。** 将操作真值与破坏前 grounding 收拢为一个高风险证据门(目标 / 当前状态 /
授权与回滚 / postcondition);把 "async 自监控"改为动作后验证义务,明确
`pending → verified` 边界。保留 compaction re-verify 与仪式合规判据;机制只覆盖可
机械检查的子集,其余由审计层关闭。
...
| 案例 | 触发 | 关键动作 |
|---|---|---|
| 1 | 第三方执行前 gate + grounding | 拦命令 → 读真实存储 → 读源码 → 推后果 |
...
| 9 | 外部人审 | assisted correction;证据成熟度 + 高风险证据门 + 动作后验证 |
...
## Event Ledger 怎样计算一次治理成效

案例在归因坐实之前,不能算 ACGM 战绩。至少记录:

- **initiator:** hook / agent / user / external audit;
- **phase:** 风险动作前、动作后,还是后续审计;
- **outcome:** blocked / corrected / allowed / unresolved / false positive;
- **verification:** 关闭事件的 postcondition 证据。

Hook 启动、skill 被调用、原则被引用,都只是活动。`Prevented` 必须有动作前 intervention,
并验证风险没有落地。若用户或外部审计先发现,就是 assisted correction,不能算 agent
自主抓到。修复必须等 postcondition 通过才完成;误报也必须保留可见。
