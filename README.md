...
[![Version: 0.3.0-rc.3](https://img.shields.io/badge/version-0.3.0--rc.3-orange.svg)](CHANGELOG.md)
...
> **Release status:** `0.3.0-rc.3` is an **unreleased testing candidate**, not a
...
> `v0.3.0-rc.3` is actually published. Its local runtime and regression suite can be
...
- versioned package identity (`0.3.0-rc.3`) and release checks;
...
The RC3 automated-regression target includes macOS, Linux, and one controlled Windows
...
For the unpublished RC3 test branch, a fresh clone plus user-scope install is:
...
it stops for manual review. Claude uninstall does not remove ACGM's persistent verified
snapshot automatically.

On Windows, RC3 accepts a directly executable native `claude.exe`. A legacy
...
After the `v0.3.0-rc.3` tag is actually published, register that immutable source and
...
> **发布状态：** `0.3.0-rc.3` 是**尚未发布的测试候选版**，不是稳定版。源码中的版本号
> 变化不代表 Git tag 或 GitHub prerelease 已发布；只有实际发布 `v0.3.0-rc.3` 后，
...
RC3 的自动回归目标包括 macOS、Linux，以及一个受控 Windows 测试 profile：
...
测试尚未发布的 RC3 分支时，全新 clone 加 user-scope 安装路径是：
...
准确一致时才算已验证成功，否则停下交给人工复核。Claude 卸载插件时不会自动删除 ACGM
自己的持久验证快照。

Windows 下 RC3 只接受可直接执行的原生 `claude.exe`。旧式 `claude.cmd` 或
`claude.bat` 会被拒绝，而不会交给隐式 shell 解析；测试本候选版前请安装当前原生
Claude Code。Windows 支持范围仍只限前述受控 Git Bash profile。

只有 `v0.3.0-rc.3` tag 实际发布后，才能注册这个不可变来源并安装保持不变的插件 ID：
